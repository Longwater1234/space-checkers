...
class Player final
{
  public:
    ...
    void showMyHunters(const std::unordered_set<int> &hunterPieces) const;
    void clearBasket();
    ...
    [[nodiscard]] bool movePiece(const int pieceId, const sf::Vector2f &destPos);
    [[nodiscard]] bool captureEnemyWith(const int pieceId, const sf::Vector2f &destPos);
  ...
  private:
    ...
    std::unordered_map<int, chk::PiecePtr> basket;
};
