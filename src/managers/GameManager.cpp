 ...
 */
const std::vector<chk::CellPtr> &GameManager::getBlockList() const
{
 ...
 */
void GameManager::handleMovePiece(const chk::PlayerPtr &player, const chk::PlayerPtr &opponent, const CellPtr &destCell,
...
bool GameManager::isHunterActive() const
{
    ...
    const int pieceId = this->getPieceFromCell(this->sourceCell.value()); // hunter pieceId
 ...
 */
int GameManager::getPieceFromCell(const int cell_idx) const
{
 ...
 */
void chk::GameManager::showForcedMoves(const chk::PlayerPtr &player, const chk::CellPtr &cell)
{
    ...
    if (moves.find(pieceId) == moves.end())
    {
        ...
        std::unordered_set<int> pieceSet;
...
bool GameManager::hasNoPossibleMoves(const chk::PlayerPtr &player) const
{
    ...
    for (const auto &cell_ptr : this->blockList)
    {
        ...
        for (const auto &destPos : candidateDests)
        {
            ...
            const auto it =
                std::find_if(this->blockList.begin(), this->blockList.end(), [&destPos](const chk::CellPtr &c) {
...
{
    const auto it = std::find_if(blockList.begin(), blockList.end(), [&cell_idx](const chk::CellPtr &cell) {
 ...
 */
void GameManager::identifyTargets(const PlayerPtr &hunter, const chk::CellPtr &singleCell)
{
    this->forcedMoves.clear();
    if (singleCell != nullptr)
    {
        // JUST CHECK AROUND this SINGLE CELL
        const int pieceId = this->getPieceFromCell(singleCell->getIndex());
 ...
 */
void GameManager::collectFrontLHS(const chk::PlayerPtr &hunter, const chk::CellPtr &cell_ptr)
{
    ...
    int deltaForward = cell_ptr->getIsEvenRow() ? 4 : 5;
    int deltaBehindEnemy = cell_ptr->getIsEvenRow() ? 5 : 4;
    ...
    const int pieceId_NW = this->getPieceFromCell(cellAheadIdx); // North West (of hunter)
    ...
    const int pieceId_SE = this->getPieceFromCell(cellBehindEnemy); // South East (of enemy)
 ...
 * @param cell_ptr current cell of hunter
 */
void GameManager::collectFrontRHS(const chk::PlayerPtr &hunter, const chk::CellPtr &cell_ptr)
{
    ...
    bool hasEnemyAhead = false;
    int deltaForward = cell_ptr->getIsEvenRow() ? 3 : 4;
    int deltaBehindEnemy = cell_ptr->getIsEvenRow() ? 4 : 3;
    int direction = +1; // up +1, down -1
    ...
    const int pieceId_NE = this->getPieceFromCell(cellAheadIdx); // North East of hunter
    ...
    const int pieceId_SW = this->getPieceFromCell(cellBehindEnemy); // South West (of enemy)
 ...
 * @param cell_ptr current cell of hunter
 */
void GameManager::collectBehindRHS(const PlayerPtr &hunter, const chk::CellPtr &cell_ptr)
{
    if (hunter->getPlayerType() == PlayerType::PLAYER_RED && cell_ptr->getPos().x >= 7 * chk::SIZE_CELL)
    {
        return;
    ...
    bool enemyOpenBehind = false;
    bool hasEnemyAhead = false;
    int deltaForward = cell_ptr->getIsEvenRow() ? 5 : 4;
    int deltaBehindEnemy = cell_ptr->getIsEvenRow() ? 4 : 5;
    ...
    const int pieceId_NW = this->getPieceFromCell(cellAheadIdx); // North west (of hunter, reverse dir)
    hasEnemyAhead = pieceId_NW != -1 && !hunter->hasThisPiece(pieceId_NW);
 ...
 * @param cell_ptr current cell of hunter
 */
void GameManager::collectBehindLHS(const PlayerPtr &hunter, const chk::CellPtr &cell_ptr)
{
    if (hunter->getPlayerType() == PlayerType::PLAYER_RED && cell_ptr->getPos().x == 0)
    {
        return;
    ...
    bool enemyOpenBehind = false;
    bool hasEnemyAhead = false;
    int deltaForward = cell_ptr->getIsEvenRow() ? 4 : 3;
    int deltaBehindEnemy = cell_ptr->getIsEvenRow() ? 3 : 4;
    int mSign = +1; // direction. up +1, down -1
    ...
    const int pieceId_NE = this->getPieceFromCell(cellAheadIdx); // North east (of hunter, reverse dir)
    hasEnemyAhead = pieceId_NE != -1 && !hunter->hasThisPiece(pieceId_NE);
